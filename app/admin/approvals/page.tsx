  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: "Total", value: filteredRows.length, color: "text-[#003768]" },
          { label: "Pending", value: totalPending, color: "text-amber-600" },
          { label: "Approved", value: totalApproved, color: "text-green-600" },
          { label: "Live Profile", value: totalLive, color: "text-blue-600" },
      ...
      <div className="rounded-3xl border border-black/5 bg-white p-6 shadow-[0_18px_45px_rgba(0,0,0,0.08)] md:p-8">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">Review partner applications and current live profile details.</p>
          </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <p className="text-sm text-slate-500">
            Showing <span className="font-semibold text-[#003768]">{Math.min(visibleCount, filteredRows.length)}</span> of <span className="font-semibold text-[#003768]">{filteredRows.length}</span> applications
          </p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-black/5">
                {loading ? (
                  <tr><td className="px-4 py-4 text-slate-600" colSpan={10}>Loading...</td></tr>
                ) : visible.length === 0 ? (
                  <tr><td className="px-4 py-4 text-slate-600" colSpan={10}>No partner applications found.</td></tr>
                ) : visible.map(row => (
                  <tr key={row.id} className="align-top hover:bg-black/[0.02]">
                    ...
                    <td className="px-4 py-4 text-slate-700">{row.role || "Partner"}</td>
                    <td className="px-4 py-4">
                      <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold capitalize ${statusPillClasses(row.status)}`}>
