      ...
      <header className="fixed inset-x-0 top-0 z-40 h-20 border-b border-black/10 bg-[#0f4f8a] text-white shadow-[0_4px_12px_rgba(0,0,0,0.18)]">
        <div className="flex h-full items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-3">
            {driverName ? <span className="hidden text-sm font-semibold text-white/95 md:block">{driverName}</span> : null}
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40 border-t border-black/10 bg-white shadow-[0_-4px_12px_rgba(0,0,0,0.08)] md:hidden">
        <div className="flex">
          <Link href="/driver/jobs"
            className={["flex flex-1 flex-col items-center gap-1 py-3 text-xs font-semibold transition", pathname === "/driver/jobs" ? "text-[#003768]" : "text-slate-500"].join(" ")}>
