...
function InfoBox({ children }: { children: React.ReactNode }) {
  return <div className="rounded-xl border border-[#003768]/10 bg-[#f3f8ff] px-4 py-3 text-sm text-[#003768]">{children}</div>;
}
...
function FieldInput({ label, value, onChange, placeholder, required }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; required?: boolean; }) {
  return (
    <div>
      <label className="block text-sm font-semibold text-[#003768] mb-1.5">{label}{required && <span className="text-red-400 ml-0.5">*</span>}</label>
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full rounded-xl border border-black/10 px-4 py-3 outline-none focus:border-[#0f4f8a] bg-white" />
    </div>
  );
}

function NavButtons({ onBack, onNext, nextLabel, saving, canSkip, onSkip }: { onBack?: () => void; onNext: () => void; nextLabel?: string; saving?: boolean; canSkip?: boolean; onSkip?: () => void; }) {
  return (
    <div className="flex items-center gap-3 mt-8">
      {onBack && <button type="button" onClick={onBack} className="rounded-full border border-black/10 px-6 py-3 font-semibold text-[#003768] hover:bg-black/5">← Back</button>}
      {canSkip && onSkip && <button type="button" onClick={onSkip} className="rounded-full border border-black/10 px-6 py-3 font-semibold text-slate-500 hover:bg-black/5">Skip for now</button>}
      <button type="button" onClick={onNext} disabled={saving} className="flex-1 rounded-full bg-[#ff7a00] py-3 font-semibold text-white shadow-[0_8px_18px_rgba(255,122,0,0.3)] hover:opacity-95 disabled:opacity-50">
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      <div className="relative">
        <input value={query} onChange={e => { setQuery(e.target.value); setOpen(true); }} onFocus={() => { if (results.length) setOpen(true); }}
          placeholder="Start typing your address…" className="w-full rounded-xl border border-black/10 px-4 py-3 outline-none focus:border-[#0f4f8a] bg-white" />
      ...
      {open && results.length > 0 && (
        <div className="absolute z-50 left-0 right-0 mt-1 rounded-2xl border border-black/10 bg-white shadow-[0_8px_24px_rgba(0,0,0,0.12)] overflow-hidden">
          {results.map((r, i) => (
            <button key={i} type="button" onClick={() => pick(r)} className="w-full text-left px-4 py-3 text-sm hover:bg-[#f3f8ff] border-b border-black/5 last:border-b-0">
  ...
  async function save() {
    ...
    try {
      ...
      const { error: e } = await supabase.from("partner_profiles").upsert({
        ...
        base_postcode: postcode || null, base_country: country, base_lat: lat, base_lng: lng, service_radius_km: radius,
      }, { onConflict: "user_id" });
  ...
  return (
    <Card title="📍 Fleet Base Location" subtitle="Set the address your vehicles operate from. This determines which customer requests you receive.">
      <div className="space-y-6">
        <InfoBox><p className="font-semibold mb-1">Why this matters</p><p>Camel Global uses your fleet base location to match you with customers within your service radius. Only requests within your radius will be sent to you — set this accurately to your depot or office location.</p></InfoBox>
...
function StepCurrency({ profile, onDone, onBack }: { profile: Profile | null; onDone: () => void; onBack: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      const { data: existing } = await supabase.from("partner_profiles").select("company_name,contact_name").eq("user_id", user.id).maybeSingle();
      const { error: e } = await supabase.from("partner_profiles").upsert({
        user_id: user.id, company_name: existing?.company_name ?? "", contact_name: existing?.contact_name ?? null, default_currency: currency,
      }, { onConflict: "user_id" });
      if (e) throw new Error(e.message);
      onDone();
    } catch (e: any) { setError(e.message); } finally { setSaving(false); }
  ...
  return (
    <Card title="💱 Bidding Currency" subtitle="Choose the currency you will use to price your bids.">
      <div className="space-y-6">
        <InfoBox><p className="font-semibold mb-1">Why this matters</p><p>All your bids will be submitted in this currency. Customers will see the equivalent in their preferred currency automatically.</p></InfoBox>
        {error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
        <div className="grid gap-4 sm:grid-cols-3">
          {[{ value: "EUR", label: "Euro", symbol: "€", desc: "Default for Spain and most of Europe" }, { value: "GBP", label: "British Pound", symbol: "£", desc: "For UK-based partners" }, { value: "USD", label: "US Dollar", symbol: "$", desc: "For US or international operations" }].map(o => (
            <button key={o.value} type="button" onClick={() => setCurrency(o.value)}
              className={`rounded-2xl border-2 p-6 text-left transition-all ${currency === o.value ? "border-[#ff7a00] bg-[#fff8f3]" : "border-black/10 bg-white hover:border-[#003768]/30"}`}>
...
function StepFleet({ onDone, onBack }: { onDone: () => void; onBack: () => void }) {
  ...
  async function addVehicle() {
    ...
    try {
      ...
      const { error: e } = await supabase.from("partner_fleet").insert({
        ...
        max_passengers: form.max_passengers, max_suitcases: form.max_suitcases, max_hand_luggage: form.max_hand_luggage, service_level: "standard", is_active: true,
      });
      if (e) throw new Error(e.message);
  ...
  async function removeVehicle(id: string) {
    await supabase.from("partner_fleet").update({ is_active: false }).eq("id", id); await load();
  }

  return (
    <Card title="🚗 Your Car Fleet" subtitle="Add the vehicle categories you can offer to customers.">
      <div className="space-y-6">
        <InfoBox><p className="font-semibold mb-1">Why this matters</p><p>Customers request specific vehicle types. You will only receive requests that match the categories you add here.</p></InfoBox>
        {error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
        ...
        {adding ? (
          <div className="rounded-2xl border border-[#003768]/10 bg-[#f3f8ff] p-5 space-y-4">
            ...
            <select value={form.category_slug} onChange={e => setForm(f => ({ ...f, category_slug: e.target.value }))} className="w-full rounded-xl border border-black/10 px-4 py-3 outline-none focus:border-[#0f4f8a]">
...
function StepDrivers({ onDone, onBack }: { onDone: () => void; onBack: () => void }) {
  ...
  return (
    <Card title="👤 Your Drivers" subtitle="Add the drivers who will deliver and collect vehicles for your customers.">
      <div className="space-y-6">
        <InfoBox><p className="font-semibold mb-1">Why this matters</p><p>Drivers meet customers at collection and return. Each driver needs an account to use the Camel Global driver app.</p></InfoBox>
        {error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
        ...
        {adding ? (
          <div className="rounded-2xl border border-[#003768]/10 bg-[#f3f8ff] p-5 space-y-4">
            ...
            {[{ key: "full_name", label: "Full name", placeholder: "Juan García", required: true }, { key: "email", label: "Email address", placeholder: "juan@valenciacars.com", required: true }, { key: "phone", label: "Phone number", placeholder: "+34 600 000 000" }].map(({ key, label, placeholder, required }) => (
              <div key={key}>
...
function StepGoLive({ profile, fleetCount, driverCount, onBack }: { profile: Profile | null; fleetCount: number; driverCount: number; onBack: () => void }) {
  ...
  return (
    <Card title="🚀 Ready to Go Live?" subtitle="Review your setup and request activation when you're ready.">
      <div className="space-y-6">
        <InfoBox><p className="font-semibold mb-1">What happens when you go live?</p><p>Once activated, customer requests within your service radius will be sent to you in real time.</p></InfoBox>
        <div>
...
export default function PartnerOnboardingPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setCompleted(done); setLoading(false);
    }
    load();
  }, []);
...
<div className="w-full">
        <div className="w-full">
          ...
          {step === "currency" && <StepCurrency profile={profile} onDone={async () => { const { data: { user } } = await supabase.auth.getUser(); if (user) await refreshProfile(user.id); complete("currency", "fleet"); }} onBack={() => setStep("location")} />}
