...
function InsuranceStatusCard({ booking }: { booking: BookingRow }) {
  ...
  return (
    <div className={`rounded-3xl border p-6 shadow-[0_18px_45px_rgba(0,0,0,0.08)] ${bothConfirmed?"border-green-200 bg-green-50":"border-amber-200 bg-amber-50"}`}>
      <div className="flex items-center justify-between">
        ...
        {bothConfirmed && <span className="rounded-full bg-green-600 px-3 py-1 text-xs font-bold text-white">✓ Confirmed</span>}
      </div>
      <p className="mt-2 text-sm text-slate-500">Driver confirms handover at delivery. Customer confirms receipt. Both must agree.</p>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className={`rounded-2xl border p-4 ${driverConfirmed?"border-blue-200 bg-blue-50":"border-slate-200 bg-slate-50"}`}>
          ...
          {driverConfirmed ? <><p className="mt-1 text-base font-bold text-blue-700">✓ Handed over</p><p className="mt-0.5 text-xs text-slate-400">{fmt(booking.insurance_docs_confirmed_by_driver_at)}</p></> : <p className="mt-1 text-sm italic text-slate-400">Not yet confirmed</p>}
        </div>
        <div className={`rounded-2xl border p-4 ${customerConfirmed?"border-green-200 bg-green-50":"border-slate-200 bg-slate-50"}`}>
          ...
          {customerConfirmed ? <><p className="mt-1 text-base font-bold text-green-700">✓ Received</p><p className="mt-0.5 text-xs text-slate-400">{fmt(booking.insurance_docs_confirmed_by_customer_at)}</p></> : <p className="mt-1 text-sm italic text-slate-400">Not yet confirmed</p>}
        </div>
      </div>
      <div className={`mt-4 rounded-2xl border p-3 text-sm font-semibold ${bothConfirmed?"border-green-200 bg-green-100 text-green-800":"border-amber-200 bg-amber-100 text-amber-700"}`}>
  ...
  return (
    <div className={`rounded-3xl border p-6 shadow-[0_18px_45px_rgba(0,0,0,0.08)] ${locked?"border-green-200 bg-green-50":"border-black/5 bg-white"}`}>
      ...
      <div className={`mt-4 rounded-2xl border p-4 ${driverConfirmed?"border-blue-200 bg-blue-50":"border-slate-200 bg-slate-50"}`}>
        ...
        {driverConfirmed&&driverFuel?<><p className="mt-1 text-xl font-bold text-[#003768]">{fuelLabel(driverFuel)}</p><FuelBar level={driverFuel}/><p className="mt-1 text-xs text-slate-400">{fmt(driverAt)}</p></>:<p className="mt-1 text-sm italic text-slate-400">Driver has not yet recorded fuel level</p>}
      </div>
      <div className={`mt-3 rounded-2xl border p-4 ${customerConfirmed?"border-green-200 bg-green-50":"border-slate-200 bg-slate-50"}`}>
        ...
        {customerConfirmed?<><p className="mt-1 text-xl font-bold text-green-700">{fuelLabel(customerFuel)} ✓</p><p className="mt-1 text-xs text-slate-400">{fmt(customerAt)}</p>{customerNotes&&<p className="mt-1 text-xs text-slate-600">Note: {customerNotes}</p>}</>:<p className="mt-1 text-sm italic text-slate-400">Waiting for customer to confirm</p>}
      </div>
      {locked?(
        <div className="mt-4 rounded-2xl border border-green-300 bg-green-100 p-3 text-sm font-semibold text-green-800">✓ Both driver and customer agree on {fuelLabel(effectiveFuel(driverFuel,savedPartnerFuel))}</div>
      ):(
        <>
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <p className="text-xs font-bold uppercase tracking-wide text-amber-700">Office override{hasOverride?` — currently set to ${fuelLabel(savedPartnerFuel)}`:""}</p>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <select value={fuelValue} onChange={e=>onFuelChange(e.target.value as FuelLevel)} disabled={locked} className="mt-1 w-full rounded-xl border border-black/10 bg-white px-3 py-2.5 text-sm outline-none focus:border-[#0f4f8a] disabled:opacity-60">
              ...
              <div className="flex items-end">
                <label className="inline-flex cursor-pointer items-center gap-2 text-sm font-medium text-[#003768]">
                  <input type="checkbox" checked={confirmed} onChange={e=>onConfirmedChange(e.target.checked)} disabled={locked} className="h-4 w-4" />Office confirms
                </label>
              </div>
            </div>
            <div className="mt-3">
              <label className="text-xs font-semibold text-[#003768]">Notes</label>
              <textarea rows={2} value={notes} onChange={e=>onNotesChange(e.target.value)} disabled={locked} className="mt-1 w-full rounded-xl border border-black/10 px-3 py-2 text-sm outline-none focus:border-[#0f4f8a] disabled:opacity-60" placeholder="Reason for override, depot drop-off, out of hours, etc." />
            </div>
            {savedPartnerAt&&<p className="mt-2 text-xs text-amber-600">Last saved: {fuelLabel(savedPartnerFuel)} at {fmt(savedPartnerAt)}</p>}
          </div>
          <button type="button" onClick={onSave} disabled={saving||locked} className="mt-4 w-full rounded-full bg-[#003768] py-3 font-semibold text-white hover:opacity-90 disabled:opacity-50 transition-opacity">
...
export default function PartnerBookingDetailPage() {
  ...
  async function loadDrivers() {
    ...
    try { const res=await fetch("/api/partner/drivers",{cache:"no-store",credentials:"include"}); const json=await res.json().catch(()=>null); if (res.ok) setDrivers((json?.data||[]).filter((d:DriverRow)=>d.is_active)); } catch { setDrivers([]); }
  ...
  return (
    <div className="space-y-6 px-4 py-8 md:px-8">
      ...
      <div className="flex items-center justify-between gap-4">
        <div><h1 className="text-3xl font-semibold text-[#003768]">Booking Detail</h1><p className="mt-1 text-slate-600">View and manage this booking.</p></div>
        <Link href="/partner/bookings" className="rounded-full border border-black/10 bg-white px-5 py-2 font-semibold text-[#003768] hover:bg-black/5">Back to Bookings</Link>
      </div>
        ...
        <div className="rounded-3xl border border-black/5 bg-white p-8 shadow-[0_18px_45px_rgba(0,0,0,0.08)]">
          ...
          <div className="mt-6 space-y-3 text-slate-700">
            ...
            <div className={`mt-4 inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold ${rateIsLive?"bg-green-100 text-green-800":"bg-slate-100 text-slate-700"}`}>
              <span className={`h-2.5 w-2.5 rounded-full ${rateIsLive?"bg-green-500":"bg-slate-400"}`}/>{rateBadgeText}{rateIsLive?" · Live rate (frankfurter.app)":""}
            </div>
      ...
      <div className="rounded-3xl border border-black/5 bg-white p-8 shadow-[0_18px_45px_rgba(0,0,0,0.08)]">
        ...
        <div className="mt-4 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-700">
          <span className="font-semibold">Current status:</span><span className="font-bold text-[#003768]">{statusLabel(bk.booking_status)}</span>
        </div>
        <form onSubmit={saveDetails} className="mt-6 space-y-5">
          <div>
            <label className="text-sm font-medium text-[#003768]">Assign driver</label>
            <select value={selectedDriverId} onChange={e=>handleDriverSelect(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 bg-white px-4 py-4 outline-none focus:border-[#0f4f8a]">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            <div><label className="text-sm font-medium text-[#003768]">Driver name</label><input value={driverName} onChange={e=>setDriverName(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-[#0f4f8a]" placeholder="John Smith"/></div>
            <div><label className="text-sm font-medium text-[#003768]">Driver phone</label><input value={driverPhone} onChange={e=>setDriverPhone(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-[#0f4f8a]" placeholder="+34 600 000 000"/></div>
            <div><label className="text-sm font-medium text-[#003768]">Vehicle</label><input value={driverVehicle} onChange={e=>setDriverVehicle(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-[#0f4f8a]" placeholder="Mercedes E-Class / AB12 CDE"/></div>
          </div>
          <div><label className="text-sm font-medium text-[#003768]">Driver notes</label><textarea rows={3} value={driverNotes} onChange={e=>setDriverNotes(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-[#0f4f8a]" placeholder="Optional notes about this assignment"/></div>
          <button type="submit" disabled={savingSection==="details"} className="rounded-full bg-[#ff7a00] px-6 py-3 font-semibold text-white shadow-[0_8px_18px_rgba(0,0,0,0.18)] hover:opacity-95 disabled:opacity-60">
      ...
      <div className="rounded-3xl border border-black/5 bg-white p-8 shadow-[0_18px_45px_rgba(0,0,0,0.08)]">
        ...
        <p className="mt-1 mb-5 text-sm text-slate-500">Exact record of who delivered and collected the vehicle and when. Stamped automatically — never editable.</p>
      ...
      <div><h2 className="mb-1 text-2xl font-semibold text-[#003768]">Insurance Documents</h2><p className="mb-4 text-sm text-slate-500">Driver confirms handover at delivery via their app. Customer confirms receipt on their portal. <span className="ml-1 text-xs text-slate-400">(Refreshes every 10s)</span></p><InsuranceStatusCard booking={bk}/></div>

      <div>
        <h2 className="mb-1 text-2xl font-semibold text-[#003768]">Fuel Tracking</h2>
        <p className="mb-4 text-sm text-slate-500">Driver records fuel level via their app. Use the office override if needed. Customer confirms to lock each stage. <span className="ml-1 text-xs text-slate-400">(Refreshes every 10s)</span></p>
        <div className="grid gap-6 xl:grid-cols-2">
