...
export default function TestBookingSignupPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const signUpDebug = {
        expectedProjectHost: customerProjectHost,
        signUpError: signUpErr ? signUpErr.message : null,
        userId: data?.user?.id || null,
        userEmail: data?.user?.email || null,
        userCreatedAt: data?.user?.created_at || null,
        sessionExists: !!data?.session,
        accessTokenIssuer: tokenPayload?.iss || null,
        accessTokenAudience: tokenPayload?.aud || null,
        accessTokenRole: tokenPayload?.role || null,
        accessTokenEmail: tokenPayload?.email || null,
      ...
      setDebug(JSON.stringify(finalDebug, null, 2));
