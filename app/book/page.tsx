...
export default function BookPage() {
  ...
  useEffect(() => {
    ...
    if (draft.dropoffAddress) setDropoffAddress(draft.dropoffAddress);
    if (draft.dropoffLat)     setDropoffLat(draft.dropoffLat);
    if (draft.dropoffLng)     setDropoffLng(draft.dropoffLng);
    if (draft.pickupAt)       setPickupAt(draft.pickupAt);
    if (draft.dropoffAt)      setDropoffAt(draft.dropoffAt);
    if (draft.passengers)     setPassengers(String(draft.passengers));
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      fetch("/api/test-booking/requests", {
        ...
        body: JSON.stringify({
          ...
          passengers: Number(d.passengers || 2),
          suitcases: Number(d.suitcases || 1),
          sport_equipment: d.sportEquipment && d.sportEquipment !== "none" ? d.sportEquipment : null,
        ...
        .then(({ ok, json }) => {
          if (!ok) throw new Error(json?.error || "Failed to create booking.");
      ...
      <div className="w-full bg-black px-6 py-16 text-white">
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="text-4xl font-black text-white md:text-5xl">
            {loading ? "Submitting your booking…" : "Confirm your booking"}
          </h1>
          <p className="mt-3 text-base font-semibold text-white/70">
            {loading
              ? "Please wait while we send your request to local car hire partners."
